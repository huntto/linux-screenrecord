...
bool XcbScreenGrab::Init(uint32_t& width, uint32_t& height, uint32_t& stride) {
    if (inited_) {
        return true;
    }
...
video::frame::Image& XcbScreenGrab::Grab(bool draw_mouse) {
    ...
    xcb_shm_get_image_cookie_t cookie = xcb_shm_get_image(connection_, screen_->root, 0, 0, image_.width, image_.height, ~0, XCB_IMAGE_FORMAT_Z_PIXMAP, shminfo_.shmseg, 0);
    xcb_shm_get_image_reply_t* reply  = xcb_shm_get_image_reply(connection_, cookie, &e);
