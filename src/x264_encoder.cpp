...
#include "../depends/libyuv/include/libyuv.h"
...
bool X264Encoder::Init(uint32_t width, uint32_t height, uint32_t fps_numerator, uint32_t fps_denominator, uint32_t bitrate) {
...
int X264Encoder::Encode(frame::Image& image, uint8_t** packet) {
    ...
    libyuv::ARGBToNV12(image.data, image.stride, input_picture_.img.plane[0], input_picture_.img.i_stride[0], input_picture_.img.plane[1], input_picture_.img.i_stride[1], image.width, image.height);

    int            frame_size = 0;
    x264_picture_t pic_out;
    x264_nal_t*    nal;
    int            i_nal;
    frame_size = x264_encoder_encode(x264_encoder_, &nal, &i_nal, &input_picture_, &pic_out);
    if (frame_size) {
